...
export function WineToolsMenu({ game, onWineTool }: WineToolsMenuProps) {
  ...
  return (
    <div className="wine-tools-menu" ref={menuRef}>
      <button className="wine-tools-menu__toggle" onClick={() => setIsOpen(!isOpen)} title="Wine Tools" disabled={!hasPrefix}>▲</button>
      {isOpen && (
        <div className="wine-tools-menu__dropdown">
          <div className="wine-tools-menu__header"><span>Ferramentas Wine</span></div>
          <div className="wine-tools-menu__section">
            {tools.map((tool) => (
              <button key={tool.id} className="wine-tools-menu__item" onClick={() => { onWineTool?.(tool.id); setIsOpen(false); }} disabled={!hasPrefix}>
          ...
          {!hasPrefix && <div className="wine-tools-menu__warning">Configure o prefixo do Wine primeiro</div>}
        </div>
      )}
    </div>
  );
