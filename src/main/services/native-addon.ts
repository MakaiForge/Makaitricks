...
export class NativeAddon {
  private static nativeModule: ProtonNativeModule | null = null;
  ...
  private static load() {
    ...
    if (!fs.existsSync(addonPath)) {
      throw new Error(`Makai Forge native addon not found at ${addonPath}`);
    }
    ...
    const nativeModule = require(addonPath) as ProtonNativeModule;
  ...
  public static processProfileImage(
    ...
    try {
      ...
      if (!normalizedImagePath || !normalizedMimeType) {
        throw new Error("Makai Forge native addon returned an invalid payload");
      }
