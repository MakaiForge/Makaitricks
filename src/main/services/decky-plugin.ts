...
export class DeckyPlugin {
  ...
  private static async getDeckyReleaseInfo(): Promise<DeckyReleaseInfo | null> {
    if (this.releaseInfo) {
  ...
  private static getPackageJsonPath(): string {
    return path.join(FORGER_DECKY_PLUGIN_LOCATION, "package.json");
  }

  private static async downloadPlugin(): Promise<string | null> {
    logger.log("Downloading Makai Forge Decky plugin...");

    const releaseInfo = await this.getDeckyReleaseInfo();
    if (!releaseInfo) return null;

    const tempDir = SystemPath.getPath("temp");
    const zipPath = path.join(tempDir, "ProtonForge.zip");
  ...
  private static async extractPlugin(zipPath: string): Promise<string> {
    logger.log("Extracting Makai Forge Decky plugin...");

    const tempDir = SystemPath.getPath("temp");
    const extractPath = path.join(tempDir, "forger-decky-plugin");
  ...
  private static async installPluginWithSudo(
    ...
    const sourcePath = path.join(extractPath, "ProtonForge");

    return new Promise((resolve, reject) => {
      const command = `mkdir -p "${DECKY_PLUGINS_LOCATION}" && rm -rf "${FORGER_DECKY_PLUGIN_LOCATION}" && cp -r "${sourcePath}" "${FORGER_DECKY_PLUGIN_LOCATION}" && chown -R ${username}: "${DECKY_PLUGINS_LOCATION}"`;
  ...
  private static async installPluginWithoutSudo(
    ...
    if (fs.existsSync(FORGER_DECKY_PLUGIN_LOCATION)) {
      await fs.promises.rm(FORGER_DECKY_PLUGIN_LOCATION, {
        recursive: true,
        force: true,
      });
    }

    await fs.promises.cp(sourcePath, FORGER_DECKY_PLUGIN_LOCATION, {
      recursive: true,
    });
  ...
  private static async updatePlugin(): Promise<void> {
    ...
    try {
      ...
      if (!zipPath) throw new Error("Failed to download plugin");
  ...
  public static async checkAndUpdateIfOutdated(): Promise<void> {
    ...
    try {
      if (!fs.existsSync(packageJsonPath)) {
        logger.log(
          "Makai Forge Decky plugin package.json not found, skipping update"
        );
        return;
      }
      ...
      if (isOutdated) {
        logger.log(
          `Makai Forge Decky plugin is outdated. Current: ${currentVersion}, Expected: ${releaseInfo.version}. Updating...`
        );

        await this.updatePlugin();
        logger.log("Makai Forge Decky plugin updated successfully");
      } else {
        logger.log(`Makai Forge Decky plugin is up to date (${currentVersion})`);
      }
    } catch (error) {
      logger.error(`Error checking/updating Makai Forge Decky plugin: ${error}`);
    }
  }
  ...
  public static async checkPluginVersion(): Promise<{
    ...
    try {
      const releaseInfo = await this.getDeckyReleaseInfo();
      if (!releaseInfo) throw new Error("Failed to fetch release info");

      if (!fs.existsSync(FORGER_DECKY_PLUGIN_LOCATION)) {
        logger.log("Makai Forge Decky plugin folder not found, installing...");
      ...
      try {
        if (!fs.existsSync(packageJsonPath)) {
          logger.log(
            "Makai Forge Decky plugin package.json not found, installing..."
          );
        ...
        if (isOutdated) {
          logger.log(
            `Makai Forge Decky plugin is outdated. Current: ${currentVersion}, Expected: ${releaseInfo.version}`
          );
        ...
        logger.error(`Error checking Makai Forge Decky plugin version: ${error}`);
        return {
          exists: false,
          outdated: true,
          currentVersion: null,
