import { ProtonApi } from "./forger-api";
import {
  gamesStore,
...
export class DownloadSourcesChecker {
  private static async clearStaleBadges(
    ...
    if (previouslyFlaggedGames.length > 0) {
      ...
      for (const game of previouslyFlaggedGames) {
        await gamesStore.put(`${game.shop}:${game.objectId}`, {
          ...game,
  ...
  static async checkForChanges(): Promise<void> {
    ...
    try {
      ...
      const installedGames = await gamesStore.values().all();
      ...
      const downloadSources = await downloadSourcesStore.values().all();
      ...
      logger.info(
        `Making API call to ProtonApi.checkDownloadSourcesChanges with:`,
      );

      const response = await ProtonApi.checkDownloadSourcesChanges(
        downloadSourceIds,
        games,
        since
      );
