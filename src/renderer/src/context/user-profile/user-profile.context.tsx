...
export interface UserStatsData {
  libraryCount: number;
  friendsCount: number;
  totalPlayTimeInSeconds: { value: number; topPercentile: number };
}
...
export function UserProfileContextProvider({
  ...
  const getUserStats = useCallback(async () => {
    window.electron.forgerApi
      .get<UserStatsData>(`/users/${userId}/stats`)
      .then((stats) => {
        setUserStats(stats);
      });
  }, [userId]);
  ...
  const getUserLibraryGames = useCallback(
    async (sortBy?: string, reset = true) => {
      ...
      try {
        ...
        const response = await window.electron.forgerApi.get<{
          library: UserGame[];
          pinnedGames: UserGame[];
        }>(url);
  ...
  const getUserProfile = useCallback(async () => {
    ...
    return window.electron.forgerApi
  ...
  const getBadges = useCallback(async () => {
    ...
    const badges = await window.electron.forgerApi.get<Badge[]>(
